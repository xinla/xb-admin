<template>
  <Form ref="form" :model="form" :rules="rules">
    ...
    <Row v-for="(item, index) of form.insurableInterest" :Key="index">
        <Col span="8">
          <Input v-model="item.scheduleName" placeholder="请输入标题" style="width:73%; margin-right: 10px;" />
        </Col>
        <Col span="16">
        <Row v-for="(unit, unique) of item.content" :Key="unique + 'x'">
          ...
          <Col span="12">
            <Input v-model="unit.algorithm" placeholder="请输入内容" style="width:73%; margin-right: 10px;" />
            <Button v-if="unique === item.content.length - 1" type="info" @click="item.content.push({})">添加</Button>
          </Col>
        </Row>
        </Col>
    </Row>
    ...
    <FormItem prop="exclusion">
      <Input type="textarea" v-model="form.exclusion" placeholder="请输入产品主要特色，不超过30字" style="width:50%;"/>
    </FormItem>
    <Divider />
...
<script>
import { getProductDescByProductId, getProductDescByDesctId, addProductDesc, updateProductDesc } from '@/api/product/desc'
